...
ServingSore<-function(DataSet, W=c(1,-1,0)){
  Clutch<-as.data.frame(rbind(filter(DataSet,DataSet$PointWinner=="P" & DataSet$PointLength=="S" & DataSet$Result %in% c("W","F") & DataSet$Server=="P"), filter(DataSet, DataSet$Result=="A" & DataSet$Server=="P")))
  Choke<-as.data.frame(rbind(filter(DataSet,DataSet$PointWinner=="O" & DataSet$PointLength=="S" & DataSet$Result %in% c("W","F") & DataSet$Server=="P"), filter(DataSet, DataSet$Result=="D" & DataSet$Server=="P")))
  Null<-as.data.frame(rbind(filter(DataSet, DataSet$ServeResult %in% c("F","S") & DataSet$PointLength %in% c("M","L") & DataSet$Server=="P")))
  #we Add an if here to catch when we run the LPcolum through Serving, sicne the situational weights of low pressure situations are 0. Here, if we find all the values are 0 we just use the dimensions in the calculation
    Score<-c(sum(Clutch$SituationalWeight)*W[1]+sum(Choke$SituationalWeight)*W[2]+sum(Null$SituationalWeight)*W[3])/(dim(DataSet)[1])
...
AppliedScoring<-function(Deuce, Ad, Joint, W=c(1,-1,0) ){
  ...
  DT<-ServingSore(filter(Deuce, Deuce$ServeLocation%in%c("L","C","T")))
  AW<-ServingSore(filter(Ad, Ad$ServeLocation=="W"))
  AB<-ServingSore(filter(Ad, Ad$ServeLocation=="B"))
  AT<-ServingSore(filter(Ad, Ad$ServeLocation%in%c("L","C","T")))
  JW<-ServingSore(filter(Joint, Joint$ServeLocation=="W"))
  JB<-ServingSore(filter(Joint, Joint$ServeLocation=="B"))
  JT<-ServingSore(filter(Joint, Joint$ServeLocation%in%c("L","C","T")))
  AppliedScores<-c(DW, DB, DT, AT, AB, AW, JT, JB, JW)
...
```{r}
MiniServeMatrix<-function(DataSet,DataType="OldSQL"){
  #Format the Matrix
  WSVector<-matrix(nrow=1,ncol=9)

  ServingData<-PServingOnly(DataSet)
  colnames(ServingData)<-c("PlayerSets","OppSets","PlayerGames","OppGames","Server", "PlayerPoints", "OppPoints", "ServeResult", "ServeLocation", "ReturnLocation", "Result", "PointWinner", "PointLength")
    
  ServingData<-as.data.frame(cbind(ServingData,SituationalWeights(ServingData)))
  colnames(ServingData)<-c("PlayerSets","OppSets","PlayerGames","OppGames","Server", "PlayerPoints", "OppPoints", "ServeResult", "ServeLocation", "ReturnLocation", "Result", "PointWinner", "PointLength", "SituationalWeights")
  DAJ<-DeuceAdCreator(ServingData)
    Deuce<-as.data.frame(DAJ[1])
    Ad<-as.data.frame(DAJ[2])
  ...
  WSVector[1,]<-AppliedScoring(Deuce,Ad,Joint)
  
  colnames(WSVector)<-c("DWide", "DBody", "DT", "AT", "ABody", "AWide", "CT", "CBody", "CWide")
...
A<-ServeTableMaker(AllData)
A
```

Building the MiniServeTable
```{r}
WSOnly<-rbind(
          MiniServeMatrix(FormatData(AllData)),
          MiniServeMatrix(FormatData(read.csv("Women's Player Data Files/AAbramyan18.csv"))),
          MiniServeMatrix(FormatData(read.csv("Women's Player Data Files/KCusick18.csv"))),
          MiniServeMatrix(FormatData(read.csv("Women's Player Data Files/HKaur18.csv"))),
          MiniServeMatrix(FormatData(read.csv("Women's Player Data Files/PMalykh18.csv"))),
          MiniServeMatrix(FormatData(read.csv("Women's Player Data Files/SSmith18.csv"))))

rownames(WSOnly)<-c("All Players","A. Abramyan", "K. Cusick", "H. Kaur", "P. Malykh", "S. Smith")

ClutchCombo<-c(0.2845972,-0.0164179,0.4046875,0.625,0.6428571,-0.2571429)

WSFinal<-cbind(ClutchCombo,WSOnly)

 #Start the gt
  WSFinalGT<-gt(as.data.frame(WSFinal), rownames_to_stub = TRUE)
  
    WSFinalGT <-
      tab_header(data = WSFinalGT,title = "Distribution of Clutch Serving Scores")
  
  #Assigning the values for our domains
  BaseCol<-c(-.2,.45)

  #Coloring the columns in order, using the values we chose earlier for highs and lows
  (WSFinalGT<-data_color(WSFinalGT,columns = c("DWide", "DBody", "DT", "AT", "ABody", "AWide", "CT", "CBody", "CWide"), colors = scales::col_numeric(palette = "RdYlGn", domain = BaseCol))) 
...
- SET UP THE DATE SYSTEM ON THE FILES
