...
courtdistfunc<-function(deuce,ad,joint)
{
  ...
  dl<-(sum(deuce$ServeLocation=="T")+sum(deuce$ServeLocation=="L"))/ndeuce
  
  ndfad<-sum(ad$ServeResult=="D")
  nad<-dim(ad)[1]-ndfad
  aw<-sum(ad$ServeLocation=="W")/nad
  ab<-sum(ad$ServeLocation=="B")/nad
  al<-(sum(ad$ServeLocation=="T")+sum(ad$ServeLocation=="L"))/nad
  ...
  jl<-(sum(joint$ServeLocation=="T")+sum(joint$ServeLocation=="L"))/njoint
...
{
  adeuce<-filter(deuce,deuce$ServeResult=="A")
  ndeuce<-dim(adeuce)[1]
  if(ndeuce==0){
    dw<-0
    db<-0
    ...
    dl<-sum(adeuce$ServeLocation=="T")/ndeuce
  }
 
  
  aad<-filter(ad,ad$ServeResult=="A")
  nad<-dim(aad)[1]
  if(nad==0){
    aw<-0
    ab<-0
    ...
    al<-sum(aad$ServeLocation=="T")/nad
  }

  
  ajoint<-filter(joint,joint$ServeResult=="A")
  njoint<-dim(ajoint)[1]
  if(njoint==0){
    jw<-0
    jb<-0
  ...
  jl<-sum(ajoint$ServeLocation=="T")/njoint
  }
...
aceprobfunc<-function(deuce,ad,joint)
{
  ...
  if(ndeuce==0){
    ...
    dl<-sum(adeuce$ServeLocation=="T")/njoint
  }
  ...
  if(nad==0){
    ...
    al<-sum(aad$ServeLocation=="T")/njoint
  }
  ...
  if(njoint==0){
    ...
    dw<-sum(ajoint$ServeLocation=="W")/njoint
    db<-sum(ajoint$ServeLocation=="B")/njoint
    dl<-sum(ajoint$ServeLocation=="T")/njoint
  }
...
shortgamewinperc<-function(deuce,ad,joint)
{
  ...
  dl<-sum(swindeuce$ServeLocation=="T")/sum(sdeuce$ServeLocation=="T")
  ...
  al<-sum(swinad$ServeLocation=="T")/sum(sad$ServeLocation=="T")
  ...
  jl<-sum(swinjoint$ServeLocation=="T")/sum(sjoint$ServeLocation=="T")
...
firstcourtdist<-function(deuce,ad,joint)
{
  ...
  dl<-(sum(fdeuce$ServeLocation=="T")+sum(fdeuce$ServeLocation=="L"))/nfdeuce
  
  fad<-filter(ad,ad$ServeResult=="F")
  nfad<-dim(fad)[1]
  aw<-sum(fad$ServeLocation=="W")/nfad
  ab<-sum(fad$ServeLocation=="B")/nfad
  al<-(sum(fad$ServeLocation=="T")+sum(fad$ServeLocation=="L"))/nfad
  ...
  jl<-(sum(fjoint$ServeLocation=="T")+sum(fjoint$ServeLocation=="L"))/nfjoint
...
secondcourtdist<-function(deuce,ad,joint)
{
  ...
  dl<-(sum(sdeuce$ServeLocation=="T")+sum(sdeuce$ServeLocation=="L"))/nsdeuce
  
  sad<-filter(ad,ad$ServeResult=="S")
  nsad<-dim(sad)[1]
  aw<-sum(sad$ServeLocation=="W")/nsad
  ab<-sum(sad$ServeLocation=="B")/nsad
  al<-(sum(sad$ServeLocation=="T")+sum(sad$ServeLocation=="L"))/nsad
  ...
  jl<-(sum(sjoint$ServeLocation=="T")+sum(sjoint$ServeLocation=="L"))/nsjoint
...
UnderPressureDistribution<-function(deuce, ad, joint){
  ...
  DL<-(sum(PDeuce$ServeLocation=="T")+sum(PDeuce$ServeLocation=="L"))/NPDeuce
  ...
  AL<-(sum(PAd$ServeLocation=="T")+sum(PAd$ServeLocation=="L"))/NPAd
  ...
  JL<-(sum(PJoint$ServeLocation=="T")+sum(PJoint$ServeLocation=="L"))/NPJoint
...
FullMatrix<-function(Dataset){
  ...
  servingdata<-cbind(pservingonly(Dataset),SituationalWeights(Dataset))
  ...
  MATRIX[6,]<-aceprobfunc(deuce,ad,joint)
