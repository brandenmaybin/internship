...
Set Points get x2
Match Points get a x3 Multiplier
...
SituationalWeights<-function(data){
  ...
  #Set Points 
  for (i in 1:N){
    if(data$player.games[i] == 5 & data$opp.games[i] == 6){Multiplier[i]<-Multiplier[i]*2+1}
    if(data$player.games[i] == 4 & data$opp.games[i] == 5){Multiplier[i]<-Multiplier[i]*2+1}
    if(data$player.games[i] == 3 & data$opp.games[i] == 5){Multiplier[i]<-Multiplier[i]*2+1}
    if(data$player.games[i] == 2 & data$opp.games[i] == 5){Multiplier[i]<-Multiplier[i]*2}
    if(data$player.games[i] == 1 & data$opp.games[i] == 5){Multiplier[i]<-Multiplier[i]*2}
    if(data$player.games[i] == 0 & data$opp.games[i] == 5){Multiplier[i]<-Multiplier[i]*2}
    if(data$player.games[i] == 6 & data$opp.games[i] == 5){Multiplier[i]<-Multiplier[i]*2+1}
    if(data$player.games[i] == 5 & data$opp.games[i] == 4){Multiplier[i]<-Multiplier[i]*2+1}
    if(data$player.games[i] == 5 & data$opp.games[i] == 3){Multiplier[i]<-Multiplier[i]*2+1}
    if(data$player.games[i] == 5 & data$opp.games[i] == 2){Multiplier[i]<-Multiplier[i]*2}
    if(data$player.games[i] == 5 & data$opp.games[i] == 1){Multiplier[i]<-Multiplier[i]*2}
    if(data$player.games[i] == 5 & data$opp.games[i] == 0){Multiplier[i]<-Multiplier[i]*2}
  }
...
fullanalysismatrix<-function(dataset,weights=c(rep(1,times=12))){
  ...
  colnames(MATRIX)<-c("Low Pressure Situations","BYU Hold", "BYU Break", "Opp Hold", "Opp Break", "Raw Combo", "Weighted Combo")
  
  #Make the lp matrix
  datasetlp<-lowpressure(dataset)
  MATRIX[,1]<-list(servingscore(datasetlp),playscore(datasetlp),clutchscore(datasetlp))
  return(MATRIX)
}
```
